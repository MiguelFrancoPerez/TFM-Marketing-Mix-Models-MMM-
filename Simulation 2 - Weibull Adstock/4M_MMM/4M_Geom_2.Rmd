...
if(Adstock_Method %in% Geometric_Methods){
  ...
  y_rang_post<-matrix(data=NA,nrow = nrow(posterior),ncol=length(rang_post))
    ...
    for (sim in 1:nrow(posterior)){
      y_rang_post[sim,]<-c( Decay_post(t=rang_post, theta = posterior[sim,paste0("theta_x[",m,"]")])  )
  }
...
#-----------------------
  ...
  for (sim in 1:nrow(posterior)){
        lines(x=data$t_ind, y = y_post[sim,], type = "l", col=adjustcolor("gray", alpha.f = 0.3))
      }
